 ...
 * Recovered from the closed vNext operator draft and wired to the Deck.
 */
...
export type WizardInputResult =
  | { type: "update"; wizard: SkillWizardState }
  | { type: "close" }
  | { type: "save"; wizard: SkillWizardState };
...
export function applyWizardInput(state: SkillWizardState, data: string): SkillWizardState {
  if (data === "escape" || data === "\x1b") return state;
...
export function handleWizardKey(wizard: SkillWizardState, data: string): WizardInputResult {
  if (data === "escape" || data === "\x1b") {
    if (wizard.step === "name") return { type: "close" };
    return { type: "update", wizard: retreatWizard(wizard) };
  }
  if ((data === "[" || data === "]") && wizard.step === "template") {
    return {
      type: "update",
      wizard: cycleWizardTemplate(wizard, data === "]" ? 1 : -1),
    };
  }
  if ((data === "enter" || data === "\r" || data === "\n") && wizardIsComplete(wizard)) {
    return { type: "save", wizard };
  }
  return { type: "update", wizard: applyWizardInput(wizard, data) };
}
...
export function cycleWizardTemplate(state: SkillWizardState, direction = 1): SkillWizardState {
  ...
  const next =
    SKILL_TEMPLATE_IDS[(index + direction + SKILL_TEMPLATE_IDS.length) % SKILL_TEMPLATE_IDS.length]!;
...
export function advanceWizard(state: SkillWizardState): SkillWizardState {
  ...
  if (state.step === "template") return { ...state, step: "triggers", error: null };
  if (state.step === "triggers") return { ...state, step: "confirm", error: null };
  return state;
}
...
export function composeWizardSkill(state: SkillWizardState): string {
  ...
  return (
    body.replace(/description: .*/, `description: ${state.description.trim()}`) + triggerBlock
  );
}

export function writeWizardSkill(
  state: SkillWizardState,
  ...
  if (existsSync(filePath)) {
    throw new SkillNameError(`Skill already exists: ${name}`);
  }
  mkdirSync(dir, { recursive: true });
  writeFileSync(filePath, composeWizardSkill({ ...state, name }), "utf8");
...
export function renderSkillWizard(theme: Theme, width: number, state: SkillWizardState): string[] {
  const title = theme.fg(
    "accent",
    `NEW SKILL WIZARD · step ${wizardStepIndex(state.step) + 1}/${WIZARD_STEPS.length}`,
  );
  const lines = [
    title,
    `Name: ${state.name || "…"}`,
  ...
  if (state.step === "confirm") {
    ...
    lines.push("type · enter next · [ ] template · backspace edit");
  }
