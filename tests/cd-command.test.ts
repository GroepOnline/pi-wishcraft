...
import {
  getCdArgumentCompletions,
  resolveCdTarget,
  runCdCommand,
} from "../src/shell/cd-command.ts";

const cdCommandSource = readFileSync(
  new URL("../src/shell/cd-command.ts", import.meta.url),
  "utf-8",
);
...
test("resolveCdTarget supports current, relative, home, and invalid directories", () => {
  ...
  assert.deepEqual(resolveCdTarget("~/project", root, home), {
    path: homeProject,
  });
  assert.deepEqual(resolveCdTarget("My\\ Folder", root, home), {
    error: `Directory does not exist: ${join(root, "My Folder")}`,
  });
  assert.match(
    resolveCdTarget("missing", root, home)?.error ?? "",
    /Directory does not exist:/,
  );
  assert.match(
    resolveCdTarget("file.txt", root, home)?.error ?? "",
    /Not a directory:/,
  );
});
...
test("cd argument completions suggest directories and quick picks", () => {
  ...
  assert.ok(
    rootCompletions.some(
      (item) => item.value === "../" && item.label === "../",
    ),
  );
  assert.ok(
    rootCompletions.some((item) => item.value === "~/" && item.label === "~/"),
  );
  assert.ok(
    rootCompletions.some(
      (item) => item.value === "child/" && item.label === "child/",
    ),
  );
  assert.ok(
    rootCompletions.some(
      (item) => item.value === "My\\ Folder/" && item.label === "My Folder/",
    ),
  );
...
test("cd command cleans up the forked session when switching is cancelled", async () => {
  ...
  assert.ok(
    switchedPath,
    "switchSession should receive the forked session path",
  );
  assert.equal(existsSync(switchedPath!), false);
  assert.deepEqual(notifications.at(-1), {
    message: "Directory change cancelled",
    type: "warning",
  });
});
...
test("cd command uses session replacement instead of private cwd mutation", () => {
  ...
  assert.match(
    cdCommandSource,
    /SessionManager\.forkFrom\(sessionFile, resolved\.path\)/,
  );
