...
test("inline wizard walks name → description → template → triggers → confirm", () => {
  ...
  for (const char of "my-skill") wizard = applyWizardInput(wizard, char);
...
test("writeWizardSkill writes the composed SKILL.md", () => {
  const root = mkdtempSync(join(tmpdir(), "wb-write-"));
  let wizard = createSkillWizard();
  for (const char of "demo-skill") wizard = applyWizardInput(wizard, char);
  wizard = advanceWizard(wizard);
  for (const char of "Demo") wizard = applyWizardInput(wizard, char);
  wizard = advanceWizard(wizard);
  wizard = advanceWizard(wizard);
  wizard = advanceWizard(wizard);
  try {
    const { filePath } = writeWizardSkill(wizard, root);
    assert.equal(filePath, join(root, "demo-skill", "SKILL.md"));
    assert.match(readFileSync(filePath, "utf8"), /description: Demo/);
  } finally {
